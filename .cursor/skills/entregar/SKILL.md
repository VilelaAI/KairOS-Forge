...
## Passo 0 — Declarar o orçamento

Antes de qualquer coisa, anuncie ao usuário e registre. É o contrato da
autonomia deste ciclo:
...
Escalação:                     2 falhas materialmente iguais, ou orçamento esgotado
...
checkpoint seguinte. Ao atingir 100%, **pause** e pergunte: encerrar com as
lacunas declaradas ou ampliar o orçamento?
...
```
1. especificar ──▶ 2. APROVAÇÃO ──▶ 3. construir ──▶ 4. validar ──┐
                                          ▲                       │ bloqueio P1
                                          └──── corrigir ◀────────┘ (máx 2)
                                                                   │ ok
                                    ┌──────────────────────────────┘
...
Se já existe SPEC aprovada, pule. Senão, rode `/kairos-forge:especificar`. As
perguntas do Pare e Pergunte chegam ao usuário **agora** — nunca responda por
ele, nunca preencha com achismo.

### 2. Aprovação da SPEC — gate humano

Apresente: objetivo, requisitos P1, não-objetivos, perguntas abertas e o plano
de tarefas. Espere **SIM / NÃO / AJUSTAR**. Sem SIM, o arco não avança.
...
### 4. Validar — e o primeiro loop

Rode `/kairos-forge:validar SPEC-NNN`.

- **Aprovado** → siga para o passo 5.
- **Aprovado com ressalvas** → siga, carregando as ressalvas para o relatório final.
- **Bloqueado** → **não devolva ao usuário.** Abra rodada de correção:
  1. Para cada achado bloqueante, identifique o agente que o relatório indicou
     (o `/validar` já nomeia: Ricardo em cobertura, Helena em segurança, Carlos
     em dados, e assim por diante).
  ...
  4. Rode `/kairos-forge:validar SPEC-NNN` de novo.

Máximo de 2 rodadas. Na terceira, **escale** (ver abaixo).

### 5. Revisar — e o segundo loop

Rode `/kairos-forge:revisar`.

- **Zero 🔴** → siga para o PR (🟠 e 🟡 viram follow-up declarado no corpo do PR).
- **Qualquer 🔴** → rodada de correção, mesmo protocolo do passo 4, roteando ao
  agente que o revisor indicou. Depois, `/kairos-forge:revisar` de novo.

Correção de segurança **nunca vira workaround** — se o único caminho for
contornar em vez de corrigir, isso é escalação, não rodada.

Se a correção tocar código de produção, o `/validar` do passo 4 **volta a valer**:
rode-o antes de reentrar na revisão. Correção que quebra requisito já validado é
...
- Corpo: objetivo da SPEC, requisitos P1 com evidência, gates rodados, ressalvas
  aceitas e follow-ups abertos, rodadas de correção consumidas.
- **Nunca** faça merge. A integração é decisão do dono do repositório.
...
A tabela de rodadas registra **também as que falharam**. Rodada revertida ou
escalada é evidência, não vergonha apagada — é o que impede a próxima entrega de
repetir a mesma tentativa.

## Escalação

Pare o arco e devolva ao usuário quando:
...
Formato da escalação — específica, com o que você já tentou:
...
- **Orçamento declarado antes da rodada 1.** Sem ele, arco vira consumo sem fim.
