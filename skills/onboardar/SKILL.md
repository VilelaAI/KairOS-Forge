...
> "Como você quer que as personas operem? Quer que eu sempre rode `garantir-qualidade` depois de `desenvolver-backend`? Quer `validar` antes de `revisar`? Quais comandos reais de teste, lint e build eu devo usar? Tem padrão de mensagem de commit específico que devo respeitar?"
...
- `onboardar` — esta skill, primeira vez
- `mapear-arquitetura` — em projeto brownfield, antes de refactor
- `especificar` — antes de feature não-trivial
- `analisar-ameacas` — antes de feature sensível
- `arquitetar-sistema` — design técnico
- `desenvolver-backend` / `desenvolver-frontend` — implementação
- `garantir-qualidade` — testes e segurança proativa
- `validar` — aceite contra SPEC
- `revisar` — revisão pré-PR
- `auditar` — sexta-feira, pontua o setup
- `evoluir` — sexta-feira, define UMA capacidade nova
...
- A skill `validar` roda os gates relevantes antes de `revisar`.
```
...
Iniciado o uso das skills da Embratur neste projeto via `onboardar`.
```
...
2. Para a próxima feature, use a skill `especificar`
3. Depois de implementar, use `validar`
4. Sexta-feira, use `auditar` para ver pontuação inicial
```
