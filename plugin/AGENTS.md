...
- `tests/` — stdlib-only `unittest` suite of the plugin itself (dogfooding — root only, not shipped): scripts as CLIs in a temporary repo, hooks executed with synthetic payloads, fault injection on state publication, trajectory sanitisation, package invariants, read-only checks (ADR-0039)
- `hooks/hooks.json` — Claude Code hooks (SessionStart banner, a once-per-session Definition-of-Done reminder on the first production file edited (ADR-0038), secret-read blocking on `Read`/`Grep` and subagent telemetry via `SubagentStart`/`SubagentStop` (ADR-0039), and the deterministic execution recorder wired at four lifecycle points — ADR-0021)
...
- Changed any `scripts/*.py` or `hooks/hooks.json` → run the suite before committing (ADR-0039): `python3 -m unittest discover -s tests -t .` — CI runs it on every PR
...
- **ADR-0039**: the plugin tests its own code — 6,157 lines of Python with zero tests get a stdlib-only `unittest` suite in CI (scripts as CLIs in a temp repo, hooks **executed** with synthetic payloads, fault injection on publication, trajectory sanitisation as an invariant, read-only by tree hash); the 8 deterministic behaviour-eval cases become tests. Alongside: per-instance subagent telemetry (`SubagentStart/Stop` + `Agent|Task|Skill` matcher — the `delegacao` branch had been dead since ADR-0021), a `leitura` guardrail class (`Read`/`Grep` on secrets) with unreadable payloads blocking, and `disable-model-invocation` on `lancar`. The suite's first run found two real defects: `--force-with-lease` blocked and `quadro.salvar` non-atomic (v0.32.0)
- **ADR-0040**: claims become facts — `concluir` requires proof of work (HEAD moved, diff under owned paths, or a declared `--sem-diff`); `executado: false` in the three report contracts separates a gate that did not run from a rejection, and `ciclo.py` neither advances nor charges a round; `varrer --pid` kills the worker (TERM, KILL) and **retains the slot** when it cannot prove death, treating zombies as dead; atomic publication with cleanup in both state files. Contracts: `contrato` 1.1, `quadro` 1.2 (v0.32.0)
