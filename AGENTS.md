...
- `scripts/ciclo.py` — Deterministic state machine for the `entregar` arc (ADR-0029). The skill executes a step and registers the outcome; the script decides the transition. Budget is enforced, not promised; `corrigindo_revisao` has exactly one outgoing edge — back to `validando`; and `registrar aprovado` is read from the validation report on disk, not taken on the agent's word
- `scripts/guardrail.py` — Deterministic guardrails: destructive commands, protected paths, SPEC integrity. Hook mode (exit 2 blocks) and CLI mode for CLIs without `PreToolUse` and for CI (ADR-0022)
