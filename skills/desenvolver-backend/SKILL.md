...
- Decidir arquitetura → use `arquitetar-sistema` primeiro.
- Componente visual ou tela → use `desenvolver-frontend`.
- Configurar CI/CD ou deploy → use `operar-infraestrutura`.
- Escrever teste isolado sem código novo → invoque Ricardo via `garantir-qualidade`.
...
Se a tarefa não tem SPEC formal mas é grande (3+ arquivos), interrompa e recomende `arquitetar-sistema` ou `especificar` antes.
