...
- Mudança de copy, cor, label → use `comunicar-produto`.
- Implementação de SPEC já decidida → use `desenvolver-backend` ou `desenvolver-frontend`.
...
| Desenho de feature | `docs/specs/SPEC-NNN-<slug>.md` | SPEC com requisitos rastreáveis (use a skill `especificar` se for o caso) |
