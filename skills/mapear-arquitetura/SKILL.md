...
description: Use when o usuário herda codebase, vai refatorar 3+ módulos, quer "extrair serviço" ou "modularizar", ou quando `auditar` apontou acoplamento alto. Produz inventário estrutural com componentes, acoplamento, duplicação de domínio, bounded contexts e plano de decomposição incremental. Read-only — salva em docs/arquitetura/MAPA-YYYY-MM-DD.md. NÃO modifica código.
---
...
- Ao herdar projeto desconhecido (segundo passo depois de `onboardar`).
- Quando `auditar` apontar "código sem dono" ou "acoplamento alto" como gap principal.
...
Para mapa estrutural completo, Diego coordena e chama os outros conforme as evidências aparecem. Estas personas vivem na skill `arquitetar-sistema` — você pode ativá-la em paralelo ou trazer o conhecimento delas pra cá.
...
- Usar a skill `especificar` para movimentos médios que viram features
```
