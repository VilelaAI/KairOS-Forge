...
Não codifique. Esta skill produz **artefato textual** — uma SPEC — que servirá de contrato para implementação depois (via `desenvolver-backend`, `desenvolver-frontend` ou execução manual).
...
Se a tarefa for primariamente de produto (escopo, priorização, MVP), Laura aciona **Camila (PM)** antes ou junto — neste caso ative também a skill `gerenciar-produto`.
...
- **Próximo passo** — sugestão de skill (`desenvolver-backend`, `desenvolver-frontend`, `validar`)
...
Antes da revisão pré-PR, use a skill `validar`.
...
2. Quando aprovada, use as skills `desenvolver-backend` / `desenvolver-frontend` conforme as tarefas
3. Após implementar: use `validar` para aceite contra SPEC
4. Antes do PR: use `revisar`
```
...
- **Não escreva código nesta skill.** Implementação é com `desenvolver-*` ou invocação direta.
