...
| Parallel team (`/mobilizar`) | ✅ Agent Teams | ✅ native subagents (`spawn_agent`, ADR-0035) | ⚠️ board runs serially | ⚠️ board runs serially |
| Agent tool allow-list | ✅ enforced | ⚠️ `shell_tool` off for advisory agents; `apply_patch` remains | ❌ | ⚠️ degrades to `readonly` |
...
| `validar` | ✅ | ✅ | ✅ | ✅ |
| `rodar` | ✅ | ✅ | ✅ | ✅ |
| `mobilizar` | ✅ Agent Teams | ✅ native subagents (ADR-0035) | ⚠️ board runs serially | ⚠️ board runs serially |
...
Codex runs `mobilizar` natively (ADR-0035). On OpenCode and Cursor there is no parallel spawn, so the skill opens the same board and runs it serially; `/kairos-forge:rodar` remains the right call for plain sequential conversation.
