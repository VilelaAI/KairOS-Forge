...
**Cuidado com homônimos**: Elisa [Naming] é diferente de Elisa (Cloud) na skill `operar-infraestrutura`. Sempre desambigue.
...
- README ou documentação técnica → use `documentar-software`.
- Definir métricas → use `medir-e-experimentar`.
- Decisão de produto sobre escopo → use `gerenciar-produto`.
