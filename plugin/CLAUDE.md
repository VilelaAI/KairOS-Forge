...
- Mudou qualquer `scripts/*.py` ou `hooks/hooks.json` → rodar a suíte antes do commit
  (ADR-0039): `python3 -m unittest discover -s tests -t .` — o CI roda em todo PR
...
| `hooks/hooks.json` | Hooks Claude Code: banner, telemetria em 4 pontos do ciclo (ADR-0021), guardrails que bloqueiam em `PreToolUse`/`PostToolUse` (ADR-0022), alerta de patinação em voo (ADR-0030), lembrete de Definition of Done **uma vez por sessão** no primeiro arquivo de produção editado (ADR-0038), leitura de segredo bloqueada em `Read`/`Grep` e telemetria de subagente por `SubagentStart`/`SubagentStop` (ADR-0039) | manual |
...
| `scripts/guardrail.py` | Guardrails determinísticos: comando destrutivo, arquivo protegido, **artefato gerado** (ADR-0037), **leitura de segredo** (ADR-0039), integridade da SPEC, PR fora de estado, contrato de relatório. Modo hook (exit 2 bloqueia; payload ilegível também bloqueia em PreToolUse) e modo CLI para os demais CLIs e o CI (ADR-0022/0032/0037/0039) | manual |
| `scripts/contrato.py` | Módulo puro dos contratos de fronteira dos relatórios: fences `kairos-critica`/`kairos-validacao`/`kairos-revisao`, coerência, prova de cobertura, independência dos críticos e `executado: false` para gate que não rodou (ADR-0040). Nunca lança, sem I/O (ADR-0032/0033/0040) | manual |
| `scripts/quadro.py` | Quadro de tarefas determinístico do `/mobilizar`: dependências, teto de onda, colisão de posse, tempo limite por tarefa, compensação em ordem inversa (Saga), recusa de encerrar com lacuna escondida, **prova de trabalho** no `concluir` e **posse do processo** no `varrer` (ADR-0035/0036/0040) | manual |
...
| `evals/comportamento-fabrica/` | Gold set dos cinco comportamentos que separam harness de pasta de prompts; os 8 casos determinísticos são testes em `tests/`, e um teste garante que nenhum caso fica sem teste (ADR-0031/0039) | manual |
| `tests/` | Suíte `unittest` só-stdlib do próprio plugin (dogfooding — só na raiz, não distribui): scripts como CLI em repositório temporário, hooks executados com payload sintético, falha injetada na publicação de estado, saneamento da trajetória, invariantes do pacote e somente-leitura (ADR-0039) | manual |
...
| `.github/workflows/ci.yml` | CI: sync sem diff pendente, `release.py check`, suíte `tests/`, segurança dos agentes, guardrail (só na raiz) | manual |
...
- **ADR-0039**: o plugin testa o próprio código — 6.157 linhas de Python sem teste viram suíte `unittest` só-stdlib no CI (script como CLI em repo temporário, hook **executado** com payload sintético, falha injetada na publicação, saneamento da trajetória como invariante, somente-leitura por hash); os 8 casos determinísticos do eval de comportamento viram testes. Junto: telemetria de subagente por instância (`SubagentStart/Stop` + matcher `Agent|Task|Skill` — o ramo `delegacao` estava morto desde o ADR-0021), classe `leitura` no guardrail (`Read`/`Grep` em segredo) com payload ilegível bloqueando, e `disable-model-invocation` no `lancar`. A primeira rodada da suíte achou dois defeitos reais: `--force-with-lease` bloqueado e `quadro.salvar` não atômico (v0.32.0)
- **ADR-0040**: afirmação vira fato — `concluir` exige prova de trabalho (HEAD avançou ou diff na posse, ou `--sem-diff` declarado); `executado: false` nos três contratos separa gate que não rodou de reprovação, e o `ciclo.py` não avança nem cobra rodada; `varrer` com `--pid` mata o worker (TERM, KILL) e **retém a vaga** se não provar que ele morreu, tratando zumbi como morto; publicação atômica com limpeza nos dois estados. Contratos: `contrato` 1.1, `quadro` 1.2 (v0.32.0)
