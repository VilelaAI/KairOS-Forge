...
| `.cursor/` | Distribuição Cursor completa: agents adaptados (readonly quando consultivo), skills espelhadas, rule `alwaysApply` (lista de skills derivada), scripts de suporte, templates (ADR-0011) e `hooks.json` ligando telemetria e guardrail aos eventos do Cursor (ADR-0042) | **gerado** por `scripts/sync-multi-cli.py` |
| `skills/<verbo>/SKILL.md` | 18 skills (compartilhadas — Claude Code e Codex leem da mesma pasta) | manual |
| `hooks/hooks.json` | Hooks Claude Code: banner, telemetria em 4 pontos do ciclo (ADR-0021), guardrails que bloqueiam em `PreToolUse`/`PostToolUse` (ADR-0022), leitura de segredo pela tool `Read` e **gate de parada** no `Stop` (ADR-0042), alerta de patinação em voo (ADR-0030) e lembrete de Definition of Done **uma vez por sessão** no primeiro arquivo de produção editado (ADR-0038) | manual |
...
| `templates/` | `CLAUDE.md.template`, `squad-fabrica.yaml`, `anti-drift.md`, `trilhas/` (blueprints de SPEC por tema, incl. pipeline de dados — ADR-0013/0037), `ci/` (gatilhos por evento pro projeto do usuário: revisar no PR, corrigir CI vermelho, auditar por cron, **triar issue** — ADR-0026/0040) | manual |
...
| `scripts/grafo.py` | Parte determinística do grafo de conhecimento (validar, diagnosticar, subgrafo, amostrar, mermaid) e a **camada de código** (ADR-0041): `codigo` extrai `importa`/`herda`/`instancia` por AST para `codigo.jsonl`, `contexto <arquivo>` dá a vizinhança de 1 salto e as entidades que citam o arquivo | manual |
...
| `scripts/telemetria.py` | Agrega o registro: `resumo` (números do `/auditar`), `sessoes`, `corroborar` (usado pelo `/validar`), `ruido` (achados de revisão descartados por revisor — ADR-0039) | manual |
| `scripts/prova.py` | Prova negativa dos testes de um diff (ADR-0039): `pre-patch` roda o teste novo contra a base num worktree descartável (tem de falhar), `testes-alterados` lista teste existente modificado ou removido com sinais de afrouxamento. Usado pelo `/validar` e pela Patrícia no `/revisar` | manual |
| `scripts/ciclo.py` | Máquina de estados determinística do arco `/entregar`: planejamento em fases, transição, orçamento dos três gates e escalação decididos por código (ADR-0029/0033); sela o digest do contrato da SPEC na aprovação e recusa resultado verde se ela mudar até `reaprovar` (contrato 1.1) | manual |
| `scripts/guardrail.py` | Guardrails determinísticos: comando destrutivo, arquivo protegido, **artefato gerado** (ADR-0037), integridade da SPEC, PR fora de estado, contrato de relatório, **teste existente** (ADR-0039 — nasce em `aviso`), e o **piso** do ADR-0042: leitura de segredo (shell e `Read`), fiação dos hooks, destruição não provável ou fora do projeto, controle da máquina, mais o **gate de parada** no `Stop`. Modo hook (exit 2 bloqueia; no Cursor, JSON no dialeto dele) e modo CLI para os demais CLIs e o CI (ADR-0022/0032/0037/0039/0042) | manual |
...
| `scripts/tests/` | Testes stdlib (`unittest`): contratos do `ciclo.py` que um runner externo depende (idempotência do `registrar`, digest da SPEC, `reaprovar` só depois da aprovação) e o piso do `guardrail.py` (cada regra, o que nega e o que libera; gate de parada; adaptador Cursor). O CI roda `python3 -m unittest discover -s scripts/tests -t scripts` | manual |
...
- **ADR-0039**: verificação é o gargalo — auditoria contra o *TLC AI Dev Flow* (Tech Leads Club, set/2026): a verificação do agente sobre o próprio trabalho não vale como prova. `prova.py` roda o teste novo contra a base sem o patch (`passou_na_base` = requisito sem evidência) e lista teste existente afrouxado no diff; classe `teste` no `guardrail.py`, a primeira que nasce em `aviso`, deixa rastro de toda edição em teste existente; `/validar` sobe os gates do barato ao caro e para no primeiro vermelho; `/revisar` relê cada achado antes de publicar e mede o próprio ruído (`descartado:` na linha + `telemetria.py ruido`); T1 da SPEC é fatia fim a fim e critério de aceite tem valor, não adjetivo. As skills `tlc-*` não entram — duplicariam o arco sem orquestração (v0.32.0)
- **ADR-0040**: porta de entrada da fábrica — o que a aula do TLC mostra sobre por onde o trabalho entra. `templates/ci/kairos-forge-triar.yml`: issue triada por Laura e Joana em `forge:pronto` / `forge:precisa-spec` / `forge:falta-informacao` / `forge:aguardar`, rótulo como fila, lock e pausa (`forge:construir` é do humano, nunca da fábrica); `rodar explorar <missão>` — POC descartável em worktree com tempo-box, saída em `docs/pocs/`, `gh pr create` bloqueado em branch `poc/*`; `DESIGN.md` na raiz é o sistema de design (formato getdesign.md, dono Pablo) e `desenhar verificar` confere contra ele; teto de apetite (ciclo → SPECs encadeadas); campo Origem na SPEC, entrada por issue e consulta a MCPs de dados antes de perguntar ao humano; seção Porta de entrada no `CLAUDE.md.template` (v0.33.0)
- **ADR-0041**: grafo de código por AST, escada de prontidão do legado e corte em sombra — os dois quadros de estrutura do TLC. `grafo.py codigo` grava a camada de código do grafo (Python por `ast`, JS/TS por regex com `tsconfig`, Go pelo `go.mod`; regenerável, única escrita do script) e `contexto <arquivo>` é o `get_architectural_context` do *Navigation Paradox* (+23 pontos onde busca textual é cega), usado por mobilizar, validar (chamadores fora do diff), revisar (5+ importadores = faixa 2), mapear-arquitetura, diagnosticar e migrar; escada de cinco degraus (build reprodutível → padronização → contexto → caracterização → harness) como pré-condição do `/migrar` e seção do `/diagnosticar`; corte em sombra com comparador e divergência zero por tempo declarado antes do percentual (v0.34.0)
- **ADR-0042**: piso de execução, gate de parada e hooks no Cursor — auditoria do `guardrail.py` contra o harness-toolkit do TLC (Felipe Rodrigues, set/2026; Elastic 2.0, então ideias, não código). Cinco regras novas no piso, sem `modos` nem `liberados`: leitura de segredo pelo shell e pela tool `Read` (com `grep` liberado), fiação dos hooks (`.cursor/hooks.json` e `.codex/hooks.json` inteiros; em `.claude/settings*.json` só o bloco `hooks`), `rm -rf` com alvo em variável, destruição fora do projeto, controle da máquina. Gate de parada no `Stop`: produção escrita sem gate verde depois não encerra em silêncio (nasce em `aviso`, nunca dispara duas vezes). `sync-multi-cli.py` gera `.cursor/hooks.json`; o payload do Cursor é reconhecido pela forma e a resposta sai no dialeto dele — telemetria e guardrail passam a existir no Cursor. Suíte `test_guardrail.py` no CI (v0.35.0)
...
| Telemetria de execução (ADR-0021) | ✅ completa (4 pontos do ciclo) | ⚠️ só SessionStart — sem trajetória útil | ❌ | ✅ via `.cursor/hooks.json` gerado (ADR-0042) |
| Guardrail em hook (ADR-0022/0042) | ✅ todas as classes | ❌ só modo CLI | ❌ só modo CLI | ✅ comando, leitura, spec, contrato e parada; classes de escrita só no modo CLI (sem hook antes da edição) |
