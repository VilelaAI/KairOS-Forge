...
Para casos específicos, prefira as skills temáticas direto (`arquitetar-sistema`, `garantir-qualidade`, etc.) — elas já trazem o time apropriado.
...
- **Não escreve SPEC formal.** Pra isso use a skill `especificar`.
- **Não audita.** Pra isso use `auditar`.
...
- Caso de uso encaixa numa skill temática → use a skill direto (`arquitetar-sistema`, `desenvolver-backend`, etc.) — ela já traz o time apropriado e com mais estrutura.
