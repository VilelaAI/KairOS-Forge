...
🔥 kairos-forge v0.35 ativo (Cursor) — 71 agentes (40 core + 31 apoio em 10 squads).
...
- Telemetria e guardrails (ADR-0021/0022/0042): `.cursor/hooks.json` liga os
  scripts de `.cursor/scripts/` aos eventos do Cursor (shell, leitura, edição,
  parada). A trajetória vai para `.agents/execucoes/` como no Claude Code; o
  que o Cursor não expõe (hook ANTES de uma edição de arquivo) fica coberto
  pelo modo CLI `guardrail.py verificar` no CI do projeto (`templates/ci/`).
...
def sincronizar_cursor():
    ...
    # 5. Hooks (ADR-0042) — o Cursor expõe eventos de ciclo de vida em
    # `.cursor/hooks.json`; os mesmos scripts do Claude Code atendem, com o payload
    # reconhecido pela forma. Caminhos relativos à raiz do projeto, como o Cursor lê.
    (CURSOR_DIR / "hooks.json").write_text(
        json.dumps(montar_hooks_cursor(), ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    return n_agents, n_skills


def montar_hooks_cursor() -> dict:
    ex = "python3 .cursor/scripts/execucao.py"
    gr = "python3 .cursor/scripts/guardrail.py"
    return {
        "version": 1,
        "_gerado": "GERADO por scripts/sync-multi-cli.py (kairos-forge) — ADR-0042. Não edite: o guardrail bloqueia.",
        "hooks": {
            "sessionStart":         [{"command": f"{ex} cursor"}],
            "beforeSubmitPrompt":   [{"command": f"{ex} cursor"}],
            "beforeShellExecution": [{"command": f"{gr} comando"}],
            "beforeReadFile":       [{"command": f"{gr} leitura"}],
            "afterShellExecution":  [{"command": f"{ex} cursor"}, {"command": f"{ex} alerta"}],
            "afterFileEdit":        [{"command": f"{ex} cursor"}, {"command": f"{gr} spec"},
                                     {"command": f"{gr} contrato"}],
            "subagentStart":        [{"command": f"{ex} cursor"}],
            "stop":                 [{"command": f"{ex} cursor"}, {"command": f"{gr} parada"}],
        },
    }
