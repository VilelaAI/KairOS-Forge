...
- `scripts/guardrail.py` — Deterministic guardrails: destructive commands, protected paths, generated artifacts, SPEC integrity, report contracts, and **pre-existing test edits** (ADR-0039 — the one class born in `aviso` mode: every edit leaves a trace in the trajectory, the warning shows once per file per session). Hook mode (exit 2 blocks) and CLI mode for CLIs without `PreToolUse` and for CI (ADR-0022/0032/0037/0039)
- `scripts/tests/` — stdlib `unittest` suite for the contracts an external runner depends on (ADR-0034): `registrar` idempotence, SPEC digest, `reaprovar` only after approval; CI runs `python3 -m unittest discover -s scripts/tests -t scripts`
