---
name: evoluir
description: Use when chega sexta-feira logo depois de `auditar`. Conduz entrevista semanal de 5 perguntas sobre a semana que passou e identifica UMA capacidade nova para construir na próxima (skill, hook, comando ou contexto). Cada execução = um item entregue na semana seguinte. NÃO constrói nada agora — produz a especificação do que será construído.
---
...
Idealmente o usuário acabou de usar a skill `auditar`. Se não usou, sugira:

> "Recomendo usar a skill `auditar` antes deste ciclo, para olharmos a foto atual. Quer que eu rode primeiro?"
...
Próximos passos:
1. Use a skill `especificar` para produzir a SPEC detalhada
