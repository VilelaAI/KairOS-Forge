...
- `exemplos/criar-projeto-demo.sh` — Scaffolds a small, **real** project (working pytest suite, declared gate) for exercising the factory locally; walkthrough in `docs/testar-localmente.md`. Real matters: on a fake project with tests that always pass, the factory looks like it works without proving anything — the exact failure mode this harness exists to prevent (repo-root only)
...
- `evals/comportamento-fabrica/` — Gold set for the five behaviors that separate a harness from a prompt folder (ADR-0031), each tied to a promise the factory already makes in writing. 8 of the 13 cases are decided by reading `.agents/execucoes/` — no model in the loop. For 3 of those, the *mechanical half* ("did the rule block?") is already decided by `guardrail.py autoteste`; the `mecanismo` field records which automation does it, and `release.py check` enforces the link in both directions so a renamed provocation or a deleted case can't rot silently. The *behavioral half* ("the agent doesn't route around it") still needs an agent — the eval does not pretend otherwise. CI keeps the set well-formed via `release.py check`
...
- `scripts/guardrail.py` — Deterministic guardrails: destructive commands, protected paths, SPEC integrity, report contracts. Hook mode (exit 2 blocks), CLI mode for CLIs without `PreToolUse` and for CI, and `autoteste` — the guardrail proving it bites in the user's own project, with their own config. Half the provocations must PASS: a guardrail that blocks everything gets switched off next week, and "5 of 5 blocked" cannot tell a healthy harness from a paranoid one. It runs in a sandbox with a copy of the config, because each provocation would otherwise write a refusal into `.agents/execucoes/` — injecting fake events into the very trajectory the factory measures itself by (ADR-0022/0032)
