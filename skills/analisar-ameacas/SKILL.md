...
Você não está corrigindo vulnerabilidades nesta skill. Você está antecipando. O artefato é um documento que vai para a SPEC ou para `decisoes/`. Mitigação concreta vira tarefa na skill `desenvolver-backend` ou ressalva em `revisar` depois.
...
- Mitigações P2 vão como ressalvas em `revisar`.
